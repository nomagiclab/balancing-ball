...
parser.add_argument("--fetch_time", action="store", type=float, default=DEFAULT_FETCH_TIME)
parser.add_argument("--initial_wait_time", action="store", type=float, default=INITIAL_WAIT_TIME)
parser.add_argument("--random_event_time", action="store", type=float, default=RANDOM_EVENT_TIME)
parser.add_argument("--number_of_trials", action="store", type=int, default=DEFAULT_TRIAL_NUM)
...
(
    ball,
    paddle
) = init_minimalistic_env_and_load_assets(p)
...
trial_id = 0
df = pd.DataFrame(columns=["trial_id", 
                            "pos_x", "pos_y", "pos_z", 
                            "lin_x", "lin_y", "lin_z",
                            "ang_x", "ang_y", "ang_z",
                            "y_roll", "x_roll"
...
while True:
    ...
    if time.time() - initial_wait_timer >= initial_wait_time:
        if time.time() - fetch_timer >= fetch_time:
            ...
            df.loc[len(df)] = [trial_id, *position, *linear_velocity, *angular_velocity, y_roll, x_roll]
            fetch_timer = time.time()
        ...
        if time.time() - random_event_timer >= random_event_time:
            linear_velocity, angular_velocity = ball.get_velocity()
            if vector_norm(linear_velocity) + vector_norm(angular_velocity) < 0.5 and distance_from_paddle_center(ball.get_position()) < 0.2:
                random_angular_velocity = np.random.uniform(low=-0.3, high=0.3, size=3).tolist()
                random_linear_velocity = np.random.uniform(low=-0.7, high=0.7, size=3).tolist()
